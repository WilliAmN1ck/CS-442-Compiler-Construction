...
//adds the name of the table being created to the tablesTable array
struct SymTab *
  ...
  while (fgets(inputBuf,MAX_LINE,fd)) {
    ...
    switch(LookupCmd(args[0])) {
      ...
      case INSERT_CMD: {
        ...
        if (entry) { //entry already in table
          struct Attributes * attr = GetAttr(entry);
          attr->value2++;
          char * scope = GetScopeNamePath(GetTable(entry));
        ...
        if (!entry || GetTable(entry) != insertTable) { //entry not in table
